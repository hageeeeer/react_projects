...
import { checkout, clearCart, deleteCart, getCart, updateCart, useCrudCart, useGetCart } from '../../useCart'
...
export default function Cart() {
  ...
  return (
    <aside className={data?.data?.numOfCartItems ? 'main-color' : '#fff'} style={isOpen ? { right: 0, transition: 'right .7s' } : { right: '-100%', transition: 'right .7s' }}>
      <i className='fa-solid fa-close p-3 fa-2x cursor-pointer' onClick={() => setIsOpen(false)}></i>
      <div className="container">


        {(data?.data?.numOfCartItems) ? <>
          <h3>shop cart:</h3>
          <h4>total cart price <span className='text-main'>{data?.data?.data?.totalCartPrice}EGP</span></h4>
          <p>total cart item <span className='fw-bolder my-3'>{data?.data?.numOfCartItems}</span></p>
          <button className='btn btn-danger ms-auto my-3 d-block' onClick={() => clearmutate()}>clear cart</button>

          {data?.data?.data?.products?.map((ele) => <div className='row align-items-center' key={ele.product._id}>
            <div className="col-md-9">
              <div className="row align-items-center">
                ...
                <div className="col-md-10">
                  ...
                  <p className='cursor-pointer' onClick={() => mutate(ele.product._id)}>Remove <i className='fa-solid fa-trash text-main '></i></p>
                </div>
              </div>
            </div>
            <div className="col-md-3 d-flex justify-content-end">
              <div>
                <button className='btn brd-btn p-1' onClick={() => mutateupdate({ id: ele.product._id, count: ele.count + 1 })}>+</button>
                <span className='mx-2'>{ele.count}</span>
                <button className='btn brd-btn p-1' onClick={() => ele.count == 1 ? mutate(ele.product._id) : mutateupdate({ id: ele.product._id, count: (ele.count) > 0 ? ele.count - 1 : ele.count })}>-</button>
              </div>
            </div>
